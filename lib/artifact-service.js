...
import { getInitiativeRow, getGeneratedArtifact, saveGeneratedArtifact, getPlaybookVersions, getConversationSummary } from './leader-store.js'
...
export async function generateAndCacheArtifact(initId, { artifactName, activityTitle, phaseName }) {
  ...
  // Enrich context: fetch synthesis, conversation summary, and playbook completion
  const [reports, conversationSummary, playbookVersions] = await Promise.allSettled([
    getSynthesisReports(initId),
    getConversationSummary(initId),
    getPlaybookVersions(initId),
  ])

  const latestReport = reports.status === 'fulfilled' && Array.isArray(reports.value) && reports.value.length
    ? reports.value[reports.value.length - 1]
    : null
  const synthesisSummary = latestReport?.themes
    ? latestReport.themes.map(t => `${t.name} (${t.pillar}): ${t.description}`).join('\n')
    : ''

  const convSummary = conversationSummary.status === 'fulfilled' ? (conversationSummary.value || '').slice(0, 800) : ''

  const latestPlaybook = playbookVersions.status === 'fulfilled' && Array.isArray(playbookVersions.value) && playbookVersions.value.length
    ? playbookVersions.value[playbookVersions.value.length - 1]
    : null
  const completionSummary = latestPlaybook?.phases ? buildPhaseCompletionLines(latestPlaybook.phases) : ''

  // Generate via Claude Sonnet (quality matters for structured docs)
