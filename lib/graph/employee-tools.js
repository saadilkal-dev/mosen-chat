...
// Bound at graph construction time — see buildEmployeeGraph
export function createEmployeeTools({ initId, empEmail }) {
  const record_employee_response = tool(
    async ({ theme, sentiment, detail, pillar }) => {
      const key = `initiative:${initId}:responses:${empEmail}`
  ...
  const request_consent = tool(
    async ({ consentId, theme, proposedText }) => {
      const key = `initiative:${initId}:consent:${empEmail}:${consentId}`
  ...
  const check_synthesis_threshold = tool(
    async ({ theme }) => {
      // Scan all consent keys for this initiative and theme
      const pattern = `initiative:${initId}:consent:*:*`
      const keys = await redis.keys(pattern)
      let count = 0
      for (const key of keys) {
        const record = await redis.hgetall(key)
        if (record && record.theme === theme && record.status === CONSENT_STATUS.GRANTED) {
          count++
        }
      }
  ...
  const generate_synthesis = tool(
    async ({ theme }) => {
      // Gather all granted consents for this theme
      const pattern = `initiative:${initId}:consent:*:*`
      const keys = await redis.keys(pattern)
      const contributions = []
      for (const key of keys) {
        const record = await redis.hgetall(key)
        if (record && record.theme === theme && record.status === CONSENT_STATUS.GRANTED) {
          contributions.push({ proposedText: record.proposedText, empEmail: record.empEmail })
        }
      }
      ...
      // Use Claude to anonymize and map to culture pillar
      const pillarMapping = {}
      CULTURE_PILLARS.forEach((p) => { pillarMapping[p] = 0 })

      const pillarResponse = await anthropic.messages.create({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 512,
        messages: [
          {
            role: 'user',
            content: `Given these anonymized employee insights about "${theme}", map each to the most relevant culture pillar from: ${CULTURE_PILLARS.join(', ')}. Return JSON: { "pillar": "PillarName", "score": 0-10 }\n\nInsights:\n${contributions.map((c) => `- ${c.proposedText}`).join('\n')}`,
          },
        ],
      })

      try {
        const parsed = JSON.parse(pillarResponse.content[0].text)
      ...
      const existing = await redis.get(`initiative:${initId}:synthesis`)
  ...
  const deliver_closed_loop = tool(
    async ({ changeDescription, closedLoopMessage, pivotId }) => {
      ...
      const key = `initiative:${initId}:closedloop:${empEmail}`
      const existing = await redis.get(key)
