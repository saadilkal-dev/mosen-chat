...
export function createEmployeeTools({ initId, empEmail }) {
    ...
    },
    {
      ...
      schema: z.object({
        ...
        proposedText: z
          .string()
          .describe(
            'The exact anonymized text that would be shared with the initiative leader. Must be written in third person — no first-person language, no identifying details.',
          ),
      }),
    },
  )
  ...
  const check_synthesis_threshold = tool(
    ...
    },
    {
      ...
      description:
        'Check whether an employee has consented to surface a theme. Call after a consent decision is made — a single consent is enough to share.',
      schema: z.object({
  ...
  const generate_synthesis = tool(
    ...
    },
    {
      ...
      description:
        'Generate an anonymized synthesis report from consented employee responses. Call when check_synthesis_threshold confirms threshold is met (≥1 contributor).',
      schema: z.object({
