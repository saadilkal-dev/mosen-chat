  ...
  return tool(
    async ({ changes, change_note }) => {
      ...
      try {
        ...
        updatedPlaybook = { ...currentVersion, modifications: text }
      }
...
export function createSuggestExperiment(initId) {
  return tool(
    async ({ phaseIndex, activityIndex }) => {
      const versions = await getPlaybookVersions(initId)
      if (!versions.length) {
        return JSON.stringify({ type: 'experiment_card', error: 'No playbook found. Generate a playbook first.' })
      }
      const current = versions[versions.length - 1]
      const phase = current.phases?.[phaseIndex]
      const activity = phase?.activities?.[activityIndex]
      if (!phase || !activity) {
        return JSON.stringify({ type: 'experiment_card', error: `Phase ${phaseIndex} or activity ${activityIndex} not found in playbook.` })
      }
      return JSON.stringify({
    ...
    },
    {
      ...
      description:
        'Suggest a specific playbook activity as the next experiment for the leader to run. Use this when recommending a concrete next action — it returns a clickable card the leader can use to navigate directly to that activity. Use phaseIndex and activityIndex from the current playbook.',
      schema: z.object({
        phaseIndex: z.number().describe('Zero-based index of the phase in the current playbook'),
        activityIndex: z.number().describe('Zero-based index of the activity within that phase'),
        reason: z.string().describe('One sentence: why this experiment makes sense right now'),
      }),
    },
  )
}
    ...
    async () => {
      ...
      if (latest.status === 'active') {
        return JSON.stringify({
          confirmed: true,
          alreadyActive: true,
          message: 'The playbook is already confirmed and active. The leader can view it in the Playbook tab.',
        })
      }
      latest.status = 'active'
      await setPlaybookVersions(initId, versions)
      await patchInitiative(initId, { playbook_generated: true, status: 'active' })
      // Fire-and-forget background pre-generation so artifacts are ready when leader opens them
      backgroundGenerateAllArtifacts(initId, latest).catch(err =>
        console.error('[confirm_playbook] background pre-gen failed:', err.message)
      )
      return JSON.stringify({
        confirmed: true,
        version: latest.version,
        message: 'Playbook confirmed and finalized. The leader can view it in the Playbook tab.',
      })
    },
    {
      name: 'confirm_playbook',
      description:
        'Confirm and finalize the draft playbook. Call this when the leader says the playbook looks good, is satisfied, or says to confirm/finalize it. Marks the draft as active and triggers background artifact generation.',
      schema: z.object({}),
    },
  )
}
