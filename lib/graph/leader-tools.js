...
import { mergePlaybookVersionSafe, findFirstIncompleteActivityIndex, diffPhaseActivitiesByTitle } from '../playbook-helpers.js'
...
No prose before or after the JSON array.`
      ...
      // Return full phase detail for the Claude-style plan approval card in chat
      const totalActivities = (playbook.phases || []).reduce((sum, p) => sum + (p.activities || []).length, 0)
...
export function createVersionPlaybook(initId) {
  return tool(
    async ({ changes, change_note }) => {
      ...
      // Carry forward completed flags from the current version for matching activity titles
      const prevCompletedByTitle = new Map()
      for (const phase of (currentVersion.phases || [])) {
        for (const act of (phase.activities || [])) {
          if (act?.completed === true && act?.title) {
            prevCompletedByTitle.set(act.title.trim(), true)
          }
        }
      }
      for (const phase of (merged.phases || [])) {
        for (const act of (phase.activities || [])) {
          if (act?.title && prevCompletedByTitle.has(act.title.trim())) {
            act.completed = true
          }
        }
      }

      // Compute activity diff between versions for the "What changed" UI
      const diff = diffPhaseActivitiesByTitle(currentVersion.phases, merged.phases)
    ...
    },
    {
      ...
      description:
        'Create a new version of the playbook when the leader accepts significant structural changes — adding or removing phases, reordering, major scope shift. For minor tweaks (rewording, changing an owner, marking complete), use update_playbook_activity instead. Frame changes naturally — never use the word "pivot" with the leader.',
      schema: z.object({
...
export function createUpdatePlaybookActivity(initId) {
  return tool(
    async ({ phaseIndex, activityIndex, updates }) => {
      const versions = await getPlaybookVersions(initId)
      if (!versions.length) return 'No playbook found.'
      const latest = versions[versions.length - 1]
      const phase = latest.phases?.[phaseIndex]
      const activity = phase?.activities?.[activityIndex]
      if (!phase || !activity) {
        return `Activity not found at [${phaseIndex}][${activityIndex}].`
      }

      if (typeof updates.title === 'string') activity.title = updates.title
      if (typeof updates.description === 'string') activity.description = updates.description
      if (typeof updates.owner === 'string') activity.owner = updates.owner
      if (typeof updates.hypothesis === 'string') activity.hypothesis = updates.hypothesis
      if (typeof updates.completed === 'boolean') activity.completed = updates.completed

      latest.lastEditedAt = Date.now()
      await setPlaybookVersions(initId, versions)

      return JSON.stringify({
        type: 'playbook_updated',
        message: `Updated "${activity.title}" in phase "${phase.name}".`,
      })
    },
    {
      name: 'update_playbook_activity',
      description:
        'Make a minor in-place edit to a specific playbook activity without creating a new version. Use for tweaks: updating a description, changing an owner, rewording, marking an activity complete/incomplete. For structural changes (adding/removing phases, reordering, major scope shift), use version_playbook instead.',
      schema: z.object({
        phaseIndex: z.number().describe('Zero-based phase index'),
        activityIndex: z.number().describe('Zero-based activity index within the phase'),
        updates: z.object({
        ...
        }).describe('Fields to update — include only what should change'),
      }),
    },
  )
}

export function createPresentOptions() {
  return tool(
    async ({ question_context, options }) => {
      return JSON.stringify({
        type: 'option_cards',
    ...
    },
    {
      ...
      description:
        "Present clickable option cards to the leader when a question has clear predefined answers. Use during brief collection for questions like team size (1-10, 10-50, 50+), timeline (a few weeks, 1-3 months, longer), or change type (structural, process, cultural, technology). Clicking an option sends it as the leader's response. NEVER use for open-ended questions like 'what is changing' or 'why'. Only use when the options genuinely cover the likely answers.",
      schema: z.object({
        question_context: z.string().describe('Brief label for what the question is about (e.g. "Timeline estimate")'),
        options: z.array(z.object({
          label: z.string().describe('Display text for the option (2-6 words)'),
          value: z.string().optional().describe('Value to send as message if different from label'),
        })).min(2).max(5).describe('2-5 predefined options'),
      }),
    },
  )
}
...
export function createLeaderTools(initId) {
  return [
    ...
    createUpdatePlaybookActivity(initId),
    createLogPivot(initId),
    createPresentOptions(),
  ]
}
