...
import {
  buildPhaseCompletionLines,
  buildTimelineAlertLine,
...
export async function markActivityComplete(initiativeId, phaseIndex, activityIndex, completed) {
  const versions = await getPlaybookVersions(initiativeId)
  if (!versions.length) throw new Error('No playbook found')
  const latest = versions[versions.length - 1]
  const activity = latest.phases?.[phaseIndex]?.activities?.[activityIndex]
  if (!activity) throw new Error(`Activity not found at [${phaseIndex}][${activityIndex}]`)
  activity.completed = completed
  await setPlaybookVersions(initiativeId, versions)
  return { version: latest, phaseIndex, activityIndex, completed }
}

export function getLeaderFlags(init) {
  const bd = init?.brief_data && typeof init.brief_data === 'object' ? init.brief_data : {}
  return bd.leader_flags && typeof bd.leader_flags === 'object' ? bd.leader_flags : {}
}

export async function mergeInitiativeLeaderFlags(initiativeId, patch) {
  const init = await getInitiativeRow(initiativeId)
  if (!init) throw new Error('Initiative not found')
  const bd = { ...(init.brief_data || {}) }
  bd.leader_flags = { ...(bd.leader_flags || {}), ...patch }
  const { error } = await sb()
    .from('initiatives')
    .update({ brief_data: bd, updated_at: new Date().toISOString() })
    .eq('id', initiativeId)
  if (error) throw error
}

export async function clearUnreadSynthesisFlag(initiativeId) {
  await mergeInitiativeLeaderFlags(initiativeId, { unread_synthesis: false })
}

export async function clearPendingPhaseCompletionFlag(initiativeId) {
  await mergeInitiativeLeaderFlags(initiativeId, { pending_phase_completion: null })
}

export async function setPendingPhaseCompletionFlag(initiativeId, payload) {
  await mergeInitiativeLeaderFlags(initiativeId, { pending_phase_completion: payload })
}

export async function getLeaderChatSnippet(initiativeId, maxChars = 1200) {
  const messages = await getLeaderChatMessages(initiativeId)
...
export async function getConversationSummary(initiativeId) {
  const { data } = await sb().from('initiative_leader_chats').select('conversation_summary').eq('initiative_id', initiativeId).maybeSingle()
  return data?.conversation_summary || ''
}

export async function saveConversationSummary(initiativeId, summary) {
  const { error } = await sb().from('initiative_leader_chats').upsert(
    {
      initiative_id: initiativeId,
...
export async function loadLeaderInitContext(initId, clerkUserId) {
  ...
  let phaseCompletionDetail = null
  let timelineAlert = null
  let nextActivity = null
  let phaseCompleted = null

  if (playbookVersions.length > 0) {
    const latestPlaybook = playbookVersions[playbookVersions.length - 1]
    const phases = latestPlaybook.phases || []

    const allActivities = phases.flatMap(p => p.activities || [])
    const completedCount = allActivities.filter(a => a.completed === true).length
    completionStats = { total: allActivities.length, completed: completedCount, remaining: allActivities.length - completedCount }

    phaseCompletionDetail = buildPhaseCompletionLines(phases)
    timelineAlert = buildTimelineAlertLine(weekNumber, phases)
    nextActivity = findFirstIncompleteActivityIndex(phases)

    // Read pending phase completion flag (set by playbook PUT route) — clear after consuming
    const leaderFlags = init.brief_data?.leader_flags || {}
    if (leaderFlags.pending_phase_completion) {
      phaseCompleted = leaderFlags.pending_phase_completion
      clearPendingPhaseCompletionFlag(initId).catch(() => {})
    }
  }
  ...
  return {
    ...
    hasSynthesis: !!latest,
    synthesisThemeCount: latest?.themes?.length || 0,
    synthesisUpdatedAt: latest?.createdAt || null,
    completionStats,
...
/**
 * After sign-in, user lands on the initiative employee experience (session + assignment).
 */
export function buildEmployeeInitiativeSignInUrl(baseUrl, initiativeId) {
  const b = (baseUrl || '').replace(/\/$/, '')
  const dest = `${b}/initiative/${initiativeId}/employee`
  return `${b}/sign-in?redirect_url=${encodeURIComponent(dest)}`
}

/** @deprecated orgId/email unused — kept for callers; returns Clerk sign-in → initiative URL */
export async function getEmployeeInviteUrl(orgId, email, initiativeId, baseUrl) {
  return buildEmployeeInitiativeSignInUrl(baseUrl, initiativeId)
}

// ── Artifact generation cache ──

export async function getGeneratedArtifact(initiativeId, artifactKey) {
  const { data } = await sb()
    .from('initiative_artifacts')
...
export async function saveGeneratedArtifact(initiativeId, artifactKey, content) {
  const { error } = await sb().from('initiative_artifacts').upsert(
    {
      initiative_id: initiativeId,
    ...
    { onConflict: 'initiative_id,artifact_key' },
  )
  if (error) throw error
}
