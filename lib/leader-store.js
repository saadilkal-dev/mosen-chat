...
export async function loadLeaderInitContext(initId, clerkUserId) {
  ...
  const flags = getLeaderFlags(init)
  ...
  if (phases.length > 0) {
    ...
    completionStats = {
      ...
      remaining: allActivities.length - completedCount,
    }
    phaseCompletionDetail = buildPhaseCompletionLines(phases)
  }

  const timelineAlert = buildTimelineAlertLine(weekNumber, phases)

  const unreadSynthesis = !!flags.unread_synthesis
  const unreadSynthesisNudge = unreadSynthesis
    ? 'There is new employee synthesis the leader has not fully incorporated into the plan yet. If they have not discussed it, surface it and offer to help version the playbook if changes are needed.'
    : ''

  const pending = flags.pending_phase_completion
  const phaseMilestoneNudge =
    pending && typeof pending === 'object' && pending.phaseName
      ? `Milestone: Phase "${pending.phaseName}" is now fully complete. Acknowledge it warmly, suggest what is next, and consider whether outreach to the team is appropriate.`
      : ''

  const hasSynthesis = !!(latest && latest.themes?.length)
  const synthesisThemeCount = latest?.themes?.length || 0
