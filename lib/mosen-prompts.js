export function leaderSystemPrompt({
  initiative_title,
  brief_summary,
  week_number,
  synthesis,
  phase_completion_detail,
  completion_stats,
  timeline_alert,
  unread_synthesis_nudge,
  phase_milestone_nudge,
} = {}) {
  const extra = [
    phase_completion_detail && `Experiment progress by phase: ${phase_completion_detail}`,
    completion_stats &&
      `Experiments overall: ${completion_stats.completed}/${completion_stats.total} completed${completion_stats.remaining != null ? `, ${completion_stats.remaining} remaining` : ''}.`,
    timeline_alert && timeline_alert,
    unread_synthesis_nudge && unread_synthesis_nudge,
    phase_milestone_nudge && phase_milestone_nudge,
  ]
    .filter(Boolean)
    .join('\n')
...
Week: ${week_number || 1}
Recent employee synthesis: ${synthesis || "No employee conversations yet"}${extra ? `\n\n${extra}` : ''}`;
}
  ...
  "phases": [
    {
      ...
      "description": "2-4 sentences: what this phase is for, sequence, and what progress looks like before moving on.",
      "startWeek": 1,
      "endWeek": 2,
      "activities": [
        {
...
- Every phase MUST include a non-empty "description" — concrete and specific to this initiative
- Set startWeek/endWeek to match the duration string when possible (integers, week numbers from initiative start)
- Frame activities as experiments with a testable hypothesis
- Set completed: false on all activities initially
  ...
  "phases": [
    {
      "name": "Phase name",
      "duration": "Timeframe (e.g., Week 1-2)",
      "description": "2-4 sentences: what this phase is for, sequence, and what done looks like.",
      "startWeek": 1,
      "endWeek": 2,
      "activities": [
        {
...
RULES:
- Return a COMPLETE playbook with all phases — not only a list of recommendations
- Every phase MUST include a substantive "description"
- Keep phases and activities that are still valid from the current playbook; preserve completed flags where still meaningful
- Every modification should trace back to a synthesis theme when possible
...
OUTPUT FORMAT — return ONLY valid JSON. No markdown fences, no prose outside JSON.
{
  "version": 1,
  "title": "Human-readable title of the artifact",
  "sections": [
    { "kind": "heading", "level": 1, "text": "Main title" },
    { "kind": "paragraph", "text": "Supporting prose." },
    { "kind": "callout", "variant": "info", "title": "Optional", "text": "Short highlight" },
    { "kind": "checklist", "title": "Optional", "items": [{ "text": "Do this", "checked": false }] }
  ]
}

RULES:
- Be specific to this initiative and the context above — avoid generic change-management boilerplate.
- Tone: warm, direct, peer-level — match Mosen voice (no jargon).`
}
