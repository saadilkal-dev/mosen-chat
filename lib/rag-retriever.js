 ...
 *
 * Fallback: if anything fails → return empty string (static knowledge dump disabled for token budget)
 */

import { embedQuery } from './embedding-service.js'
import { getSupabase } from './supabase.js'
import { LEADER_CONTEXT } from './mosen-knowledge.js'

const RAG_TOP_K = 8            // more chunks for richer context
const RAG_THRESHOLD = 0.50     // slightly looser to catch related concepts
const MAX_CONTEXT_CHARS = 5000 // more room for substantive knowledge blocks
...
export async function retrieveContext(userMessage, persona = 'leader') {
  ...
  try {
    ...
    let block = `=== CHANGE PRINCIPLES — APPLY THESE TO THE CURRENT CONVERSATION ===
The following passages are from the books that ground your thinking. Read them and actively use them:
- Identify which principle, pillar, or pattern from these passages is at play in what the leader just said
- Name the pattern naturally — "What you're describing sounds like a trust gap..." not "According to LAABS..."
- Draw specific suggestions from these principles, not generic advice
- If a passage is directly relevant, weave it into your response as a wise colleague would — not as a citation
...
export async function ragStatus() {
  ...
  try {
    ...
    return {
      available: (count || 0) > 0,
      chunks: count || 0,
      reason: count > 0 ? 'ready' : 'no embeddings ingested yet — see RAG_SETUP.md',
    }
  } catch (err) {
...
function staticFallback(persona) {
  console.warn('[RAG] No vector results — static fallback disabled to save tokens')
  return ''
}
