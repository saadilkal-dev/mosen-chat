...
function pickFirstName(o) {
  return pickFromRow(o, ['first name', 'firstname', 'first_name', 'given name', 'fname'])
}

function pickLastName(o) {
  return pickFromRow(o, ['last name', 'lastname', 'last_name', 'surname', 'family name', 'lname'])
}

/** Full display name for roster: "First Last", or legacy single "name" column. */
function pickFullName(o) {
  const first = pickFirstName(o)
  const last = pickLastName(o)
  if (first || last) {
    return [first, last].filter(Boolean).join(' ').trim()
  }
  return pickFromRow(o, ['name', 'full name', 'employee name', 'display name'])
}
...
export function employeesFromObjects(rows) {
  ...
  return rows
    .map((raw) => {
      ...
      const firstName = pickFirstName(o)
      const lastName = pickLastName(o)
      const name = pickFullName(o)
      return {
        name,
        firstName,
        lastName,
